...
{   
    ...
    while(true)
    {
        ...
        if (message[0] == 'N')
        {
            std::thread(initNotification, client_addr, ipAndPortToNickname, nicknameToIPPort, message).detach();
        }
        // else if (message[0] == 'L')
        // {
        //     std::thread(sendListUsers, socketFD, client_addr, ipAndPortToNickname, message).detach();
        // }
        // else if (message[0] == 'M')
        // {
        //     std::thread(forwardMessage, socketFD, client_addr, ipAndPortToNickname, nicknameToIPPort, message).detach();
        // }
        // else if (message[0] == 'W')
        // {
        //     std::thread(forwardDiffusionMessage, socketFD, client_addr, ipAndPortToNickname, nicknameToIPPort, message).detach();
        // }
        // else if (message[0] == 'Q')
        // {
        //     std::thread(closeSession, socketFD, client_addr, ipAndPortToNickname, nicknameToIPPort, message).detach();
        // }
    }
}
