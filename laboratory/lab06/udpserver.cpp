...
{   
    ...
    while(true)
    {
        ...
        if (message[0] == 'N')
            initNotification(client_addr, ipAndPortToNickname, nicknameToIPPort, message);
        else if (message[0] == 'L')
            sendListUsers(socketFD, client_addr, ipAndPortToNickname, message);
        else if (message[0] == 'M')
            forwardMessage(socketFD, client_addr, ipAndPortToNickname, nicknameToIPPort, message);
        else if (message[0] == 'W')
            forwardDiffusionMessage(socketFD, client_addr, ipAndPortToNickname, nicknameToIPPort, message);
        else if (message[0] == 'Q')
            closeSession(socketFD, client_addr, ipAndPortToNickname, nicknameToIPPort, message);
    }
}
